...
if [[ "${SERVICE_NAME}" == "userprofiles" || "${SERVICE_NAME}" == "identity" ]]; then
    kubectl delete configmap "${service}-settings"
    kubectl create configmap "${service}-settings" --from-file="${config_filename}"="${env}-${service}-azure-settings.json" -o yaml  > "${ENV}-config.yml"
else
    kubectl delete configmap "${service}-settings"
    kubectl create configmap "${service}-settings" --from-file="${config_filename}"="${env}-${service}-settings.json" -o yaml  > "${ENV}-config.yml"
fi;
