...
### createCollection(collection_id: `CollectionId`, schema: `NFTSchema`, royalties_schedule: `Option<RoyaltiesSchedule>`)
...
### directSale(collection_id: `CollectionId`, token_id: `TokenId`, buyer: `Option<AccountId>`, payment_asset: `AssetId`, fixed_price: `Balance`)
- **interface**: `api.tx.nft.directSale`
- **summary**:   Sell an NFT to specific account at a fixed price `buyer` optionally, the account to receive the NFT. If unspecified, then any account may purchase `asset_id` fungible asset Id to receive as payment for the NFT `fixed_price` ask price Caller must be the token owner 
