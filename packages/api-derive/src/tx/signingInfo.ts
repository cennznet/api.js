...
import {FALLBACK_PERIOD, MAX_FINALITY_LAG, MORTAL_PERIOD} from '@polkadot/api-derive/tx/constants';
...
function latestNonce(api: ApiInterfaceRx, address: string): Observable<Index> {
  return api.query.system.accountNonce(address).pipe(first()) as Observable<Index>;
}
...
export function signingInfo(
  ...
  return (address: string, nonce?: AnyNumber | Codec, era?: IExtrinsicEra | number): Observable<Result> =>
    combineLatest([
      ...
      isUndefined(nonce) ? latestNonce(api, address) : of(api.registry.createType('Index', nonce)),
      // if no era (create) or era > 0 (mortal), do block retrieval
      isUndefined(era) || (isNumber(era) && era > 0) ? signingHeader(api) : of(null),
    ]).pipe(
      map(([nonce, header]) => ({
        header,
        mortalLength: MORTAL_PERIOD.div(
