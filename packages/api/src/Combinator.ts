...
export default class Combinator {
    ...
    constructor(fns: Array<CombinatorFunction | [Array<any>, CombinatorFunction]>, callback: CombinatorCallback) {
        ...
        this._subscriptionIds = fns.map(
            (input, index): Promise<number> => {
                const [args, fn] = Array.isArray(input) ? input : [[], input];
    ...
    static unsubscribe(id: number): Promise<any> {
        ...
        return combinator ? combinator.unsubscribe() : Promise.resolve(false);
    }
        ...
        return Promise.all(
            this._subscriptionIds.map((subscriptionPromise, index) => {
                ...
                return !unsubscribe
                    ...
                    : subscriptionPromise
                          .then(subscriptionId => unsubscribe(subscriptionId))
                          .then(() => true)
                          .catch(() => false);
            })
        );
