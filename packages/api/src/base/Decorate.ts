...
import decorateDerive, {ExactDerive} from '@cennznet/api-derive';
...
import augmentObject from '@polkadot/api/util/augmentObject';
import {decorateSections, DeriveAllSections} from '@polkadot/api/util/decorate';
import {extractStorageArgs} from '@polkadot/api/util/validate';
...
export default abstract class Decorate<ApiType extends ApiTypes> extends Events {
  ...
  protected readonly _options: ApiOptionsBase;
   ...
   * ```
   */
  constructor (options: ApiOptionsBase, type: ApiTypes, decorateMethod: DecorateMethod<ApiType>) {
    super();
