...
import { StorageEntryPromiseOverloads } from '@polkadot/api/types'
import { ApiOptionsBase, DecorateFn, DecorateMethodOptions, ObsInnerType, UnsubscribePromise, VoidFn } from '../types';
...
import Combinator, { CombinatorCallback, CombinatorFunction } from '@polkadot/api/promise/Combinator';
...
function decorateSubscribe<Method extends DecorateFn<ObsInnerType<ReturnType<Method>>>> (method: Method, actualArgs: unknown[], resultCb: Callback<Codec>): UnsubscribePromise {
  return new Promise<VoidFn>((resolve, reject): void => {
    ...
    const subscription: Subscription = method(...actualArgs).pipe(
      ...
      typeof setImmediate === 'undefined'
        ? setTimeout(() => resultCb(result, undefined) as void, 0)
        : setImmediate(() => resultCb(result, undefined) as void);
    });
  });
}
...
export default class ApiPromise extends ApiBase<'promise'> {
   ...
   * @param options options that is passed to the class contructor. Can be either [[ApiOptionsBase]] or a
   ...
   * ```
   */
  public static create (options?: ApiOptionsBase): Promise<ApiPromise> {
    return new ApiPromise(options).isReady;
  }

  /**
   * @description Creates an instance of the ApiPromise class
   * @param options Options to create an instance. This can be either [[ApiOptionsBase]] or
   * an [[WsProvider]].
   * @example
   * <BR>
   *
