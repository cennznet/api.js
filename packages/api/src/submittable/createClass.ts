...
import {
  ...
  SignatureOptions,
  ISubmittableResult,
  IExtrinsicEra, Signer
} from '@cennznet/types/types';
import { ApiInterfaceRx, ApiTypes, SignerResult } from '../types';
import { AddressOrPair, SignerOptions, SubmittablePaymentResult, SubmittableResultResult, SubmittableResultSubscription, SubmittableThis } from '../types';
import { SubmittableExtrinsic as SubmittableExtrinsicBase } from "@polkadot/api/types";
...
import { filterEvents, isKeyringPair } from '@polkadot/api/util';
import ApiBase from '../base';
import SubmittableResult from '@polkadot/api/submittable/Result';
import {AnyNumber, AnyU8a} from "@polkadot/types/types/helpers";
import {Option} from "@polkadot/types/codec";
import Doughnut from "@cennznet/types/Doughnut";
import {IRuntimeVersion} from "@polkadot/types/types/interfaces";
...
export default function createClass <ApiType extends ApiTypes> ({ api, apiType, decorateMethod }: SubmittableOptions<ApiType>): Constructor<SubmittableExtrinsicBase<ApiType>> {
  // an instance of the base extrinsic for us to extend
  const ExtrinsicBase = api.registry.createClass('Extrinsic');

  return class Submittable extends ExtrinsicBase implements SubmittableExtrinsicBase<ApiType> {
        ...
        (): Observable<RuntimeDispatchInfo> =>
          api.derive.tx.signingInfo(address, allOptions.nonce, allOptions.era).pipe(
            ...
            switchMap((signingInfo): Observable<RuntimeDispatchInfo> => {
              ...
              const signOptions = this.#makeEraOptions(allOptions, signingInfo);
    ...
    #makeSignOptions = (options: Partial<SignerOptions>, extras: { blockHash?: Hash; era?: ExtrinsicEra; nonce?: Index}): SignatureOptions => {
      let doughnut: Option<Doughnut>;
      if (options.doughnut)
      {
        doughnut = this.registry.createType('Option<Doughnut>', {value: options.doughnut});
      }
      return {
