...
type Creator<ApiType extends ApiTypes> = (extrinsic: Call | Uint8Array | string) => SubmittableExtrinsicBase<ApiType>;

let Submittable: Constructor<SubmittableExtrinsicBase<any>>;
...
export default function createSubmittable<ApiType extends ApiTypes> (apiType: ApiTypes, api: ApiInterfaceRx, decorateMethod: ApiBase<ApiType>['_decorateMethod']): Creator<ApiType> {
  return (extrinsic: Call | Extrinsic | Uint8Array | string): SubmittableExtrinsicBase<ApiType> => {
    ...
    return new Submittable(api.registry, extrinsic) as SubmittableExtrinsicBase<ApiType>;
  };
}
