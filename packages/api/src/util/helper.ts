...
export async function awaitDepositClaim(
  ...
  claim: Erc20DepositEvent | { tokenAddress?: any; amount?: any; beneficiary?: any } | string | Uint8Array,
  signer: KeyringPair
): Promise<ClaimDeposited> {
  const nonce = await api.rpc.system.accountNextIndex(signer.address);
  return new Promise((resolve, reject) => {
    api.tx.erc20Peg.depositClaim(depositTxHash, claim).signAndSend(signer, { nonce }, ({ status, events }) => {
      let eventClaimId;
      if (status.isInBlock) {
        for (const {
