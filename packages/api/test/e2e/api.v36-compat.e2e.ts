...
describe('runtime v36 compatibility', () => {
  ...
  describe('Make transactions', () => {
    ...
    it('Decodes historical block', async () => {
      ...
      expect(block.block.header.hash.toHex()).toBe('0xc7f53f0070d9d9b593106fcd766ed6f2c85be44ca5e0511b9944e91d08f2164f');
    ...
    it('Queries historical block events', async done => {
      ...
      const checkBlockEvents = async function test(blockHash: string) {
        ...
        expect(events[1].event.section).toEqual('system');
        expect(events[1].event.method).toEqual('ExtrinsicSuccess');
        // GA transfer extrinsic success
        expect(events[2].event.section).toEqual('genericAsset');
        expect(events[2].event.method).toEqual('Transferred');
