...
describe('EnhancedAssetId', () => {
  describe('decoding', () => {
    ...
    it('from number (reserve asset)', () => {
      const assetId = new AssetId(registry, CENTRAPAY.id);
      const enhanced = new EnhancedAssetId(registry, CENTRAPAY);
      expect(enhanced.toString()).toEqual(assetId.toString());
    ...
    it('from number (not reserve asset)', () => {
      const enhanced = new EnhancedAssetId(registry, MAX_RESERVE_ID + 1);
    ...
    it('from symbol', () => {
      const assetId = new AssetId(registry, CENTRAPAY.id);
      const enhanced = new EnhancedAssetId(registry, CENTRAPAY.symbol);
      expect(enhanced.toString()).toEqual(assetId.toString());
    });

    it('from invalid symbol', () => {
      expect(() => new EnhancedAssetId(registry, 'xxx')).toThrow(/invalid asset/);
    });
    ...
    it('from BN', () => {
      const enhanced = new EnhancedAssetId(registry, new AssetId(registry, CENTRAPAY.id));
      expect(enhanced.toString()).toEqual(CENTRAPAY.id.toString());
      expect(enhanced.isReserve()).toBeTruthy();
      expect(enhanced.symbol).toEqual(CENTRAPAY.symbol);
      expect(enhanced.decimals).toEqual(CENTRAPAY.decimals);
  ...
  it('encoding the same', () => {
    const assetId = new AssetId(registry, CENTRAPAY.id);
    const enhanced = new EnhancedAssetId(registry, CENTRAPAY.id);
