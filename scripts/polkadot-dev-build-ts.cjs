...
// this build script is taken from:
// https://github.com/polkadot-js/dev/blob/master/packages/dev/scripts/polkadot-dev-build-ts.cjs
// and lightly modified to work here
...
const EXT_CJS = '.cjs'; // .cjs
const EXT_ESM = '.js'; // .mjs
...
async function buildBabel (dir, type) {
  ...
  await babel({
    babelOptions: {
      configFile: type === 'esm'
        ? path.join(__dirname, '../node_modules/@polkadot/dev/config/babel-config-esm.cjs')
        : configs.find((f) => fs.existsSync(f)) || path.join(__dirname, '../node_modules/@polkadot/dev/config/babel-config-cjs.cjs')
    },
    cliOptions: {
      extensions: ['.ts', '.tsx'],
      filenames: ['src'],
